...
class RepRequestInherit(models.Model):
    _inherit = 'sfa.rep.request'
    ...
    def _search(self, args, offset=0, limit=None, order=None):
        ...
        if teams:
            ...
            domain = [('salesman', 'in', list(set(team_member_ids + ([current_employee.id] if current_employee else []))))]
            args = expression.AND([args or [], domain])

        elif current_employee:  # If no teams but employee exists, only show their records
            domain = [('salesman', 'in', [current_employee.id])]
            args = expression.AND([args or [], domain])
