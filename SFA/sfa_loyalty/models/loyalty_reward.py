...
class SFALoyaltyReward(models.Model):
    _name = 'sfa.loyalty.reward'
    ...
    credit_journal = fields.Many2one('account.journal',domain="[('type', '=', 'sale'), '|', ('company_id', '=', company_id), ('company_id', '=', False)]")
    ...
    # Discount rewards
    discount = fields.Float('Credit', default=10)
    amount = fields.Float('Amount', default=10)
    ...
    _sql_constraints = [
        ...
        ('product_qty_positive', "CHECK (reward_type != 'amount' OR reward_product_qty > 0)",
            'The reward product quantity must be strictly positive.'),
        ('discount_positive', "CHECK (reward_type != 'percentage' OR discount > 0)",
    ...
    def _compute_multi_product(self):
        for reward in self:
            ...
            reward.multi_product = reward.reward_type == 'amount' and len(products) > 1
            reward.reward_product_ids = reward.reward_type == 'amount' and products or self.env['product.product']
    ...
    @api.depends('reward_type', 'reward_product_id', 'discount_mode',
                 'discount', 'currency_id', 'all_discount_product_ids')
    ...
    @api.depends('reward_type', 'discount_mode')
    def _compute_is_global_discount(self):
        for reward in self:
            reward.is_global_discount = reward.reward_type == 'percentage' and\
