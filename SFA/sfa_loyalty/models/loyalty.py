...
class SFALoyalty(models.Model):
    _name = 'sfa.loyalty'
    _description = 'SFA Loyalty'
    _order = 'sequence'
    _rec_name = 'name'
    ...
    company_id = fields.Many2one('res.company', 'Company', default=lambda self: self.env.company,domain="[('id', 'in', allowed_company_ids)]")
    ...
    rule_ids = fields.One2many('sfa.loyalty.rule', 'program_id', 'Conditional rules', copy=True,
         compute='_compute_from_program_type', readonly=False, store=True)
    reward_ids = fields.One2many('sfa.loyalty.reward', 'program_id', 'Rewards', copy=True,
         compute='_compute_from_program_type', readonly=False, store=True)
    communication_plan_ids = fields.One2many('sfa.loyalty.mail', 'program_id', copy=True,
         compute='_compute_from_program_type', readonly=False, store=True)
    ...
    coupon_ids = fields.One2many('sfa.loyalty.card', 'program_id')
    ...
    program_type = fields.Selection([
        ...
        ('promotion', 'Rebate Promotions'),
        ('ewallet', 'eWallet'),
        ('promo_code', 'Discount Code'),
        ('buy_x_get_y', 'Buy X Get Y'),
        ('next_order_coupons', 'Next Order Coupons')],
        default='promotion', required=True,readonly=True
    )
    ...
    def _inverse_mail_template_id(self):
        for program in self:
            ...
            if not program.mail_template_id:
                ...
                program.communication_plan_ids = self.env['sfa.loyalty.mail'].create({
    ...
    def _compute_coupon_count(self):
        read_group_data = self.env['sfa.loyalty.card']._read_group([('program_id', 'in', self.ids)], ['program_id'], ['__count'])
        ...
        return {
            'coupons': {
                ...
                'communication_plan_ids': [(5, 0, 0), (0, 0, {
                    'trigger': 'create',
                    'mail_template_id': (self.env.ref('sfa.loyalty.mail_template_loyalty_card', raise_if_not_found=False) or self.env['mail.template']).id,
                })],
            },
            'promotion': {
                'applies_on': 'current',
            ...
            'gift_card': {
                ...
                'communication_plan_ids': [(5, 0, 0), (0, 0, {
                    'trigger': 'create',
                    'mail_template_id': (self.env.ref('sfa.loyalty.mail_template_gift_card', raise_if_not_found=False) or self.env['mail.template']).id,
                })],
            },
            'loyalty': {
                'applies_on': 'both',
            ...
            'next_order_coupons': {
                ...
                'communication_plan_ids': [(5, 0, 0), (0, 0, {
                    'trigger': 'create',
                    'mail_template_id': (
                        self.env.ref('sfa.loyalty.mail_template_loyalty_card', raise_if_not_found=False)
    ...
    def _compute_from_program_type(self):
        ...
        grouped_programs = defaultdict(lambda: self.env['sfa.loyalty'])
        for program in self:
        ...
        if 'reward_ids' in vals and self._fields['reward_ids'].convert_to_cache(vals['reward_ids'], self):
            ...
            if 'program_type' in vals:
                ...
                for program in self:
                    ...
                    super(SFALoyalty, program).write(vals)
                return True
        else:
            return super().write(vals)
