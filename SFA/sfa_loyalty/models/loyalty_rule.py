...
class SFALoyaltyRule(models.Model):
    _name = 'sfa.loyalty.rule'
    ...
    def default_get(self, fields_list):
        ...
        if 'program_type' in self.env.context:
            ...
            program_default_values = self.env['sfa.loyalty']._program_type_default_values()
    ...
    program_id = fields.Many2one('sfa.loyalty', required=True, ondelete='cascade')
    ...
    customer_domain = fields.Char(default="[]")

    filtered_products = fields.Many2many('product.product', string='Filtered Products',
                                         compute='_compute_filtered_records')
    filtered_customers = fields.Many2many('res.partner', string='Filtered Customers',
                                          compute='_compute_filtered_records')
    qty_or_purchase = fields.Selection([
        ('quantity', 'Quantity'),
        ('purchase', 'Value'),
    ],required=True,default='quantity',string='Qty Or Value')



    @api.depends('product_domain', 'customer_domain')
    def _compute_filtered_records(self):
        for record in self:
            try:
                # Get the domain conditions
                product_filter = literal_eval(record.product_domain)
                customer_filter = literal_eval(record.customer_domain)

                # Retrieve the records based on domains
                record.filtered_products = self.env['product.product'].search(product_filter)
                record.filtered_customers = self.env['res.partner'].search(customer_filter)
            except Exception:
                record.filtered_products = False
                record.filtered_customers = False
    ...
    salesman = fields.Many2many('hr.employee', string="Salesman")
    route = fields.Many2many('sfa.journey.assignment'
                             )
    ...
    @api.onchange('qty_or_purchase')
    def _onchange_qty_or_purchase(self):
        if self.qty_or_purchase == 'quantity':
            self.minimum_amount = False
        elif self.qty_or_purchase == 'purchase':
            self.minimum_qty = False
    ...
    def _constrains_code(self):
        ...
        if len(mapped_codes) != len(set(mapped_codes)) or \
                self.env['sfa.loyalty.rule'].search_count(
        ...
        if self.env['sfa.loyalty.card'].search_count([('code', 'in', mapped_codes)]):
