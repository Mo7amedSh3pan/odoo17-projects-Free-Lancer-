...
class SFALoyaltyGenerateWizard(models.TransientModel):
    _name = 'sfa.loyalty.generate.wizard'
    _description = 'Generate Coupons'

    program_id = fields.Many2one('sfa.loyalty', required=True, default=lambda self: self.env.context.get('active_id', False) or self.env.context.get('default_program_id', False))
    ...
    def generate_coupons(self):
        ...
        self.env['sfa.loyalty.card'].create(coupon_create_vals)
        return True
